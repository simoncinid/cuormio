...
def normalize_phone(raw_phone):
    ...
    if not digits:
        return None
    # Rimuovi prefisso internazionale doppio
...
# Estrai info con fallback telefono
...
def extract_order_info(order):
    ...
    prodotto = order.line_items[0].title if order.line_items else None
    return {
...
def check_run_script():
    ...
    # Martedì -> weekday()==1
    if now_rome.weekday() == 1 and now_rome.hour == 23 and now_rome.minute == 59:
        print("[SCHED] Eseguo run_script() alle 23:59 martedì.", flush=True)
        run_script()
    else:
